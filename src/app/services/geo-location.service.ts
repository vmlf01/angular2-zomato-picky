...
export class GeoLocationService {
    ...
    _getLocationName(location: GeoLocation): Observable<GeoLocation> {
        const qsParams = new URLSearchParams();
        qsParams.set('lat', location.latitude.toString());
        qsParams.set('lon', location.longitude.toString());

        return this.http.get('https://developers.zomato.com/api/v2.1/cities', {
            search: qsParams,
            headers: new Headers(
                {
            ...
            .map(locationName => Object.assign({}, location, { name: locationName }));
    }
